...
function getDefaultAmount(
  ...
  /*
   * Start at the offer minimum.
   * This lets the user explicitly increase
   * the trade amount.
   */
  return minimum > 0
    ? minimum
...
function handleTradeConfirmation(
  ...
  /*
   * Notify the router / future Orders module.
   */
  window.dispatchEvent(
    new CustomEvent(
      "cap:trade-created",
...
function reserveTradeFunds(
  ...
  if (
    ...
    throw new Error(
      "CAP Marketplace: wallet state is unavailable."
    );
  }
...
function createOrder(
  ...
  /*
   * Store the complete trade context.
   *
   * This prevents the future Orders / Escrow modules
   * from needing to reconstruct the trade from partial data.
   */
  const order = {
...
function createId(
  ...
  return `${prefix}_${Date.now()}_${Math
    .random()
    .toString(36)
    .slice(2, 10)}`;
}
