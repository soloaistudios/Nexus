...
/**
 * Keep the central state and the visible marketplace
 * controls synchronized.
 *
 * The marketplace opens on "Buy CAP", so the state must
 * also represent the user-facing BUY side.
 */
function normalizeMarketplaceFilters() {
   ...
   * UI meaning:
   *
   * state side "sell" = counterparty sells CAP
   *                     = user can BUY CAP
   *
   * state side "buy"  = counterparty buys CAP
   *                     = user can SELL CAP
   */
  ...
  const offers =
    state.marketplace.offers.filter(
      (offer) => {
         ...
         * so the user can BUY CAP.
         *
         * A BUY offer means the counterparty buys CAP,
         * so the user can SELL CAP.
         */
