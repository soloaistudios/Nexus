...
export function initCAPDebug() {
  ...
  /*
   * IMPORTANT:
   * Install the error listeners immediately.
  ...
  debugState.listenersInstalled = true;

  /*
   * The document body may not exist yet.
   * Build the UI when it becomes available.
   */
  ensureDebugUI();
  ...
  addDebugEntry({
    ...
    source: "CAP DEBUG",
    message:
      "Global debug monitor initialized before application startup.",
  });
}
...
function installConsoleErrorCapture() {
  ...
  console.error =
    (...args) => {
      ...
      /*
       * Keep normal DevTools console behavior.
       */
      try {
        ...
        /* Ignore console forwarding failure. */
      }
    };
}
...
export function addDebugEntry({
  ...
  /*
   * Also mirror important diagnostics
  ...
  for (
    ...
    item.innerHTML = `
      ...
      ${
        entry.details
          ? `
            <pre class="cap-debug-entry-details">${escapeHTML(
              entry.details
            )}</pre>
          `
          : ""
